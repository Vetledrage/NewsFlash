...
class ScrapeService(
    ...
    fun scrapeAndSave(inputUrl: String) {
        ...
        val externalId = extractArticleIdOrNull(normalizedUrl)
        if (externalId == null) {
        ...
        if (text.length < 200) {
            log.info("Skipping article {} because extracted text is too short ({} chars): {}", externalId, text.length, normalizedUrl)
            return
        }
        ...
        val source = "VG"
        val articleId = "$source:$externalId"
